...
class EMT_simulator:
        ...
        for i in setpoint_steps_list[:]:
            if setpoint_steps[i]["type"] == "P":
                ...
                for elem_plot in range(self.time_array_plot.shape[0]):
                    if self.time_array_plot[elem_plot] > step_time: # check -- should be at the exact time
                        self.Pref_plot[elem_plot:] = step_value
                        break
                for elem in range(self.time_array.shape[0]):
                    if self.time_array[elem] > step_time: # check -- should be at the exact time
    ...
    def create_vars_symbols(self, names, suffix):
        return sp.Matrix([sp.Symbol(f"{name}_{suffix}") for name in names])
    
    def create_parameter_symbols(self):
        return sp.Matrix([sp.symbols("valpha_next vbeta_next")])

    def create_sim_variables(self, vars_list):
        vars_xn0 = self.create_vars_symbols(vars_list, 'xn0')
        vars_xn1 = self.create_vars_symbols(vars_list, 'xn1')
        parameters_next_symbols = self.create_parameter_symbols()
        self.calculate_lambda_functions(vars_xn0, vars_xn1, parameters_next_symbols)
        self.create_args_buffer(vars_xn0, vars_xn1, parameters_next_symbols)

    def calculate_lambda_functions(self, vars_xn0, vars_xn1, parameters_next_symbols):
        residual_function = self.solving_system_control(vars_xn0, vars_xn1, parameters_next_symbols)
        Jac_symbolic = residual_function.jacobian(vars_xn1)
        all_syms = list(vars_xn0) + list(vars_xn1) + list(parameters_next_symbols)
        self.res_func = sp.lambdify(all_syms, residual_function, modules="numpy")
        self.jac_func = sp.lambdify(all_syms, Jac_symbolic, modules="numpy")

    def create_args_buffer(self, vars_xn0, vars_xn1, parameters_next_symbols):
        assert vars_xn0.shape[0] == vars_xn1.shape[0]
        no_total_vars = len(list(vars_xn0)) + len(list(vars_xn1)) + len(list(parameters_next_symbols))
        self.args_buffer          = np.empty(no_total_vars)
        self.first_division_args  = len(list(vars_xn0))
        self.second_division_args = len(list(vars_xn0)) + len(list(vars_xn1))
    
    def eval_f_control(self, x):
        return np.asarray(self.res_func(*x), dtype=np.float64).ravel()

    def eval_jacobian_control(self, x):
        return np.asarray(self.jac_func(*x), dtype=np.float64)
    ...
    def dif_pll_system(self, x1, vq):
        x1_dot = vq
        x2_dot = self.Kp_pll*vq + self.Ki_pll*x1 + self.wN

        return sp.Matrix([x1_dot, x2_dot])
    ...
    def park_transformation_pll(self, theta_pll, valpha, vbeta, vq):
        res_q =  -sp.sin(theta_pll)*valpha + sp.cos(theta_pll)*vbeta - vq

        return res_q
    
    def solving_system_control(self, states_xn, states_xn1, sim_parameters_xn1):
        valpha_xn1, vbeta_xn1 = sim_parameters_xn1

        x1_xn, x2_xn, vq_xn = states_xn
        x1_xn1, x2_xn1, vq_xn1 = states_xn1

        f1 = sp.Matrix([x1_xn1, x2_xn1]) - sp.Matrix([x1_xn, x2_xn]) - self.ponder_trapz * (self.dif_pll_system(x1_xn, vq_xn) + self.dif_pll_system(x1_xn1, vq_xn1))
        f2 = self.park_transformation_pll(x2_xn1, valpha_xn1, vbeta_xn1, vq_xn1)

        return sp.Matrix([f1, f2])
    
    def newton_raphson(self, states_xn, valbe_xn1):
        new_states = states_xn.copy()
        self.args_buffer[self.second_division_args:] = valbe_xn1
        self.args_buffer[:self.first_division_args] = states_xn
        for _ in range(10):
            self.args_buffer[self.first_division_args:self.second_division_args] = new_states
            f_val = self.eval_f_control(self.args_buffer)
            # print(current_time, f_val)
            if np.max(np.abs(f_val)) < 1e-12:
                return new_states
            
            J_val = self.eval_jacobian_control(self.args_buffer)

            new_states += np.linalg.solve(J_val, -f_val)
    ...
    def solver_control(self, iteration, states_xn, i2_elec, v_elec):
        ...
        states_pll = self.newton_raphson(np.concatenate([states_xn[0:2], states_xn[9:10]]), valbe_xn1)
        states_xn[0:2] = states_pll[0:2]
    ...
    def save_steady_state(self, file_name, elec_vars, ctrl_vars):
        np.savez(
            f"./sim_setups/init_{file_name}.npz",
            elec_states=elec_vars,
            ctrl_states=ctrl_vars
        )

    def run_algorithm(self, params_saver=False):
        ...
        if params_saver:
            self.save_steady_state("init_sim_v2", state_iteration_elec, state_iteration_control)
    ...
    def park_transfo_voltage_postprocess(self, theta, va, vb, vc):
        vd =  2/3 * (np.cos(theta)*va + np.cos(theta-self.wfs_shift)*vb + np.cos(theta+self.wfs_shift)*vc)/self.V_base*self.peakLN_rmsLL
        vq =  -2/3 * (np.sin(theta)*va + np.sin(theta-self.wfs_shift)*vb + np.sin(theta+self.wfs_shift)*vc)/self.V_base*self.peakLN_rmsLL
        return np.column_stack([vd, vq])
    ...
    def save_arrays(self, file_name):
        ...
        np.savez(
            f'./Saved_trajectories/Sim_4events/nr_{file_name}.npz',
...
if __name__ == "__main__":
    ...
    sim_step = 50e-6
    plot_step = 500e-6
    parameters_model = "params_pv_4events"

    simulator = EMT_simulator(sim_time, sim_step)
    simulator.upload_parameters(plot_step, parameters_model)

    control_vars = ["x1", "x2", "vq"]
    simulator.create_sim_variables(control_vars)

    simulator.run_algorithm(params_saver=False)
    ...
    if show_plot:
        ...
        y_plotting = [
                    [(simulator.phidq_cc_array[:, 0], r"$phi_{{cc}}$", "b", "-")],
                    [(simulator.phipq_pc_array[:, 0], r"$phi_{{pc}}$", "b", "-")],
                    [(simulator.udq_array[:, 0], r"$u_{{d}}$", "b", "-"), (simulator.udq_array[:, 1], r"$u_{{q}}$", "b", "-")],
                    [(simulator.S2_array[:, 0], r"$P_{{ref}}$", "b", "-")],
                    [(simulator.idq_star_array[:, 0]-simulator.idq2_array[:, 0], r"$i^*_{{d}}$", "r", "-")],
                    [(simulator.idq2_array[:, 0], r"$i_{{d2}}$", "r", "-")],
                    [(simulator.idq_star_array[:, 0], r"$i^*_{{d}}$", "r", "-")],
                    ]
