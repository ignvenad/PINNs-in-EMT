...
class EMT_simulator:
    ...
    def solver_control(self, iteration, states_xn, i2_elec, v_elec):
        ...
        states_xn[0:2] = self.compute_dif_pll(states_xn[0], states_xn[1], states_xn[9], self.vq_prev)
        states_xn[1] %= 2*np.pi # clip theta_pll
    ...
    def compute_dif_pll(self, x1_xn, x2_xn, vq_xn, vq_past):
        x1_xn1 = x1_xn + self.ponder_trapz * (vq_past + vq_xn)
        x2_xn1 = x2_xn + self.ponder_trapz * (self.Kp_pll*vq_past + self.Ki_pll*x1_xn + self.Kp_pll*vq_xn + self.Ki_pll*x1_xn1 + 2*self.wN)

        return np.stack([x1_xn1, x2_xn1])
    ...
    def save_arrays(self, file_name):
        ...
        np.savez(
            f'./Saved_trajectories/Sim_4events/del_{file_name}.npz',
...
if __name__ == "__main__":
    ...
    if show_plot:
        plotter = Sim_Plotter()
        # y_plotting = [
        #             # [(simulator.pll_vars_array[:, 0], r"$x_{{1}}$", "b", "-")],
        #             [(simulator.pll_vars_array[:, 1], r"$x_{{2}}$", "b", "-")],
        #             [(simulator.udq_array[:, 0], r"$u_{{d}}$", "b", "-"), (simulator.vdq_array[:, 0], r"$v_{{d}}$", "r", "-")],
        #             [(simulator.udq_array[:, 1], r"$u_{{q}}$", "b", "-"), (simulator.vdq_array[:, 1], r"$v_{{q}}$", "r", "-")],
        #             [(simulator.idq_star_array[:, 0], r"$i_{{d}}$", "r", "-"), (simulator.idq_star_array[:, 1], r"$i_{{q}}$", "b", "-")],
        #             [(simulator.S1_array[:, 0], r"$P_{{1}}$", "r", "-"), (simulator.S2_array[:, 0], r"$P_{{2}}$", "b", "-"), (simulator.S3_array[:, 0], r"$P_{{3}}$", "b", "-")],
        #             [(simulator.S1_array[:, 1], r"$Q_{{1}}$", "r", "-"), (simulator.S2_array[:, 1], r"$Q_{{2}}$", "b", "-"), (simulator.S3_array[:, 1], r"$Q_{{3}}$", "b", "-")],
        #             # [(simulator.va_array[:, 0]/simulator.V_base*simulator.peakLN_rmsLL, r"$v_{{a}}$", "b", "-"), (simulator.va_array[:, 1]/simulator.V_base*simulator.peakLN_rmsLL, r"$v_{{b}}$", "b", "-")]
        #             ]
